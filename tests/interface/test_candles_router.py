"""Testes do mural de velas: 503 sem banco configurado, CRUD básico com o
repositório em memória, validação e rate limit."""

from __future__ import annotations

from collections.abc import Iterator

import pytest
from fastapi.testclient import TestClient

from app.infrastructure.candles.in_memory_repository import InMemoryCandleRepository
from app.interface.candles.router import _rate_limiter
from app.main import app


@pytest.fixture()
def client() -> Iterator[TestClient]:
    """TestClient próprio (não o `module`-scoped de test_routers): cada
    teste aqui manipula `app.state.candle_repository`, então precisa de
    isolamento por teste, não compartilhado."""
    with TestClient(app) as test_client:
        yield test_client


@pytest.fixture(autouse=True)
def reset_rate_limiter() -> Iterator[None]:
    """O limitador é um singleton do módulo — sem isto, um teste "gasta" a
    janela de outro."""
    _rate_limiter._last_seen.clear()
    yield
    _rate_limiter._last_seen.clear()


def test_should_return_503_when_database_not_configured(client: TestClient) -> None:
    """Sem `DATABASE_URL`, o lifespan real deixa `candle_repository` em `None`."""
    # Given
    client.app.state.candle_repository = None

    # When
    response = client.get("/api/velas")

    # Then
    assert response.status_code == 503
    assert response.json()["code"] == "VELAS_INDISPONIVEL"


def test_should_reject_creation_when_database_not_configured(
    client: TestClient,
) -> None:
    # Given
    client.app.state.candle_repository = None

    # When
    response = client.post(
        "/api/velas", json={"nome": "Maria", "tipo": "jesus"}
    )

    # Then
    assert response.status_code == 503


def test_should_create_and_list_vela_when_repository_is_configured(
    client: TestClient,
) -> None:
    # Given
    client.app.state.candle_repository = InMemoryCandleRepository()

    # When
    create_response = client.post(
        "/api/velas",
        json={"nome": "Maria", "intencao": "Pela saúde da família", "tipo": "jesus"},
    )

    # Then
    assert create_response.status_code == 201
    created = create_response.json()
    assert created["nome"] == "Maria"
    assert created["intencao"] == "Pela saúde da família"
    assert created["tipo"] == "jesus"
    assert created["id"] == 1

    # When
    list_response = client.get("/api/velas")

    # Then
    assert list_response.status_code == 200
    body = list_response.json()
    assert body["total"] == 1
    assert body["itens"][0]["nome"] == "Maria"


def test_should_accept_and_expose_city_and_state(client: TestClient) -> None:
    # Given
    client.app.state.candle_repository = InMemoryCandleRepository()

    # When
    response = client.post(
        "/api/velas",
        json={"nome": "Ana", "tipo": "aparecida", "cidade": "Aparecida", "estado": "SP"},
    )

    # Then
    assert response.status_code == 201
    body = response.json()
    assert body["cidade"] == "Aparecida"
    assert body["estado"] == "SP"


def test_should_accept_valid_email_but_never_return_it(client: TestClient) -> None:
    # Given
    client.app.state.candle_repository = InMemoryCandleRepository()

    # When
    response = client.post(
        "/api/velas",
        json={"nome": "Carlos", "tipo": "sao_jose", "email": "carlos@example.com"},
    )

    # Then
    assert response.status_code == 201
    assert "email" not in response.json()

    # When
    list_response = client.get("/api/velas")

    # Then
    assert "email" not in list_response.json()["itens"][0]


def test_should_reject_malformed_email(client: TestClient) -> None:
    # Given
    client.app.state.candle_repository = InMemoryCandleRepository()

    # When
    response = client.post(
        "/api/velas",
        json={"nome": "Beatriz", "tipo": "jesus", "email": "nao-e-email"},
    )

    # Then
    assert response.status_code == 422


def test_should_accept_creation_without_intention(client: TestClient) -> None:
    # Given
    client.app.state.candle_repository = InMemoryCandleRepository()

    # When
    response = client.post(
        "/api/velas", json={"nome": "João", "tipo": "aparecida"}
    )

    # Then
    assert response.status_code == 201
    assert response.json()["intencao"] is None


def test_should_reject_blank_name(client: TestClient) -> None:
    # Given
    client.app.state.candle_repository = InMemoryCandleRepository()

    # When
    response = client.post("/api/velas", json={"nome": "   ", "tipo": "jesus"})

    # Then
    assert response.status_code == 422


def test_should_reject_unknown_candle_type(client: TestClient) -> None:
    # Given
    client.app.state.candle_repository = InMemoryCandleRepository()

    # When
    response = client.post(
        "/api/velas", json={"nome": "Ana", "tipo": "verde-limao"}
    )

    # Then
    assert response.status_code == 422


def test_should_rate_limit_second_creation_from_same_client(
    client: TestClient,
) -> None:
    # Given
    client.app.state.candle_repository = InMemoryCandleRepository()
    first = client.post("/api/velas", json={"nome": "Pedro", "tipo": "sao_jose"})
    assert first.status_code == 201

    # When
    second = client.post("/api/velas", json={"nome": "Pedro", "tipo": "sao_jose"})

    # Then
    assert second.status_code == 429
    assert second.json()["code"] == "RATE_LIMITED"


def test_should_paginate_velas_newest_first(client: TestClient) -> None:
    # Given
    repo = InMemoryCandleRepository()
    client.app.state.candle_repository = repo
    import asyncio

    from app.domain.candles.entities import CandleType, NewCandle

    for name in ["Primeira", "Segunda", "Terceira"]:
        asyncio.run(repo.create(NewCandle(name=name, candle_type=CandleType.SAINT_JUDE_THADDEUS)))

    # When
    response = client.get("/api/velas", params={"limit": 2, "offset": 0})

    # Then
    body = response.json()
    assert body["total"] == 3
    assert [item["nome"] for item in body["itens"]] == ["Terceira", "Segunda"]
