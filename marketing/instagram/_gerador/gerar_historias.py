...
Toda história segue a mesma espinha dorsal: fato → desenvolvimento →
desfecho → LIÇÃO (a aplicação pra vida de quem está lendo, não só o relato
histórico) → CTA. Sem a lição, é uma aula de história; com ela, é uma
reflexão de fé — que é o que a página existe pra oferecer.
"""
import sys
...
def filho_prodigo() -> None:
    d = OUT / "filho-prodigo"
    total = 6
    make_cover_slide("filho_prodigo", "uma história para refletir", "O Filho Pródigo", "arraste para ler →", d / "1-capa.jpg", 0, total)
    make_text_slide(
        "Parte 2 de 6",
    ...
    make_text_slide(
        ...
        '"Este meu filho estava morto e voltou à vida" (Lc 15,24). A festa não veio depois de um pedido '
        "de desculpas perfeito — veio do simples fato de ele ter voltado.",
        d / "4.jpg", 3, total,
    )
    make_text_slide(
        "A lição",
        "Se você sente que se afastou demais — da fé, de alguém, de quem você queria ser — a parábola "
        "não pede um pedido de perdão perfeito. Pede só a volta. Deus não espera no portão contando os "
        "seus erros: Ele corre primeiro.",
        d / "5-licao.jpg", 4, total,
    )
    make_cta_slide(
        "Leia a parábola completa (Lucas 15, 11-32) e mais sobre as parábolas de Jesus em",
        "compendio-catolico.com", d / "6-cta.jpg", 5, total,
    )
...
def paixao_em_4_atos() -> None:
    ...
    total = 7
    make_cover_slide("cruz", "sexta-feira santa", "A Paixão em 4 atos", "arraste para ler →", d / "1-capa.jpg", 0, total)
    make_text_slide(
        "1. Getsêmani",
    ...
    make_text_slide(
        "A lição",
        "Toda sexta-feira parece definitiva quando você está vivendo ela. Nenhuma era, pra Ele. Se o que "
        "você carrega agora parece o fim, a Páscoa é a prova de que Deus sempre tem um terceiro dia "
        "guardado — mesmo quando você não consegue ver daqui.",
        d / "6-licao.jpg", 5, total,
    )
    make_cta_slide(
        "Mais sobre a Semana Santa e a liturgia do ano todo em",
        "compendio-catolico.com", d / "7-cta.jpg", 6, total,
    )
